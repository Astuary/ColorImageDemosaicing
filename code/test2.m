left_I=imread('../data/disparity/tsukuba_im1.jpg');
right_I=imread('../data/disparity/tsukuba_im5.jpg');
left_I=mean(left_I,3);
right_I=mean(right_I,3);
I_disp=zeros(size(left_I), 'uint8');
disp_range=55;
h_block_size=9;
...
col=size(left_I,2);
min_diff= Inf;
disp_cost= min_diff*ones(col,2*disp_range+1,'uint8');
penalty=0.4;
...
for m =1:row
  ...
  Index= zeros(size(disp_cost), 'uint8');
  	c_p = disp_cost(end, :);
    for j = col-1:-1:1
